...
void BuildWindows(float bank[W_BANK][WIN_LEN+1]) {
      ...
      Gen7(bank[W_RAMPDOWN], s, 5); }
    {
        ...
        // DECAY, two stages summed. A single exponential cannot be both snappy and
        // long-tailed: how fast it falls and how far it falls are the same number, so
        // lengthening the tail always softens the initial drop. A fast component plus a
        // slow one behaves like a plucked string instead: the transient bites, then a low
        // tail hangs on. Measured against the previous single exponential, this reaches
        // -6 dB at 11.9% of the grain instead of 17.6% (snappier) while reaching -20 dB at
        // 50.6% instead of 43.6% (longer), with the mean level unchanged from the original.
        // The t^6 terminal taper only bites near the very end, forcing the true zero that
        // keeps grain boundaries from clicking without disturbing the body of the curve.
        int m = WIN_LEN+1-a;
        const double A = 0.65, T1 = 0.05, T2 = 0.40;
        const double n0 = A + (1.0 - A);
        for (int i = 0; i < m; ++i) {
            double t = (double)i/(m-1);
            double v = (A*exp(-t/T1) + (1.0-A)*exp(-t/T2))/n0;
            v *= pow(0.001, pow(t, 6.0));
            ft[a+i] = (float)v;
        }
...
// ============================ BANDPASS ============================
void Bandpass::Process(float* buf, int n, float hz, float mix) {
    const float Q = 1.0f;                  // low resonance: colour, not whistle
