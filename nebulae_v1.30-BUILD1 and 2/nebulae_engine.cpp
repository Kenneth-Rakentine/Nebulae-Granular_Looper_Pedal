...
void Engine::Init(float sr, float* a, float* b, int cap) {
    ...
    // SIZE LFO shape: skew 0.35 mirrors LFO 1, so the rise is quick and the fall is long.
    // Curve 0.6 pushes the output toward its extremes, so instead of rocking evenly it
    // snaps away from centre, then blossoms and lingers before easing back. Bipolar, so
    // loop length swings both above and below wherever Size is set.
    lfo2_.Init(sr); lfo2_.SetSkew(0.35f); lfo2_.SetCurve(0.6f);
    fsh_.Init(sr);
    bph_.Init(sr);
    bpf_.Init(sr);
    dc_.Init(sr, 12.0f);
    ...
    //                 primary  shift   page3
    catch_[0].Init(0.0f,  0.0f,  0.5f);   // POT1: Start   / LFO Rate     / BANDPASS (0.5 = off)
    catch_[1].Init(1.0f,  0.0f,  0.0f);   // POT3: Size    / Grain Spray  / SIZE LFO DEPTH
    catch_[2].Init(0.55f, 0.5f,  1.0f);   // POT4: Density / Overlap      / SHIFT MIX (1.0 = full wet)
    catch_[3].Init(1.0f,  0.0f,  0.0f);   // POT6: Blend   / LFO Depth    / GRAIN PITCH RAND (0 = off)
    catch_[4].Init(0.5f,  0.0f,  0.0f);   // POT2: Speed / BLEND LFO DEPTH / LFO 2 RATE
    ...
    held_rate_=0.0f;  held_depth_=0.0f; held_speed_=0.5f;
    held_pitch_=0.5f; held_outlvl_=0.0f;
    held_bdep_=0.0f;    // blend LFO depth, off
    held_rate2_=0.0f; held_depth2_=0.0f; lfo2_prev_=0.0f;
...
void Engine::ResetAltParams() {
    ...
    held_rate_ = 0.0f; held_bdep_ = 0.0f; held_spray_ = 0.0f;
...
void Engine::Process(const float* in, float* out, int n, Controls& c) {
    ...
    else if (page == 1) { held_rate_  = p1; held_bdep_  = p2; held_spray_ = p3;
                          held_ovl_   = p4; held_outlvl_= p5; held_depth_ = p6; }
    else                { /* page 3: SHIFT up + FS1 held. No panel labels. */
                          held_rate2_ = p2; held_depth2_ = p3;
                          held_bpf_   = p1; held_fsmix_ = p4; held_fshift_ = p5;
                          held_prand_ = p6; }

    in_gain_ = 1.0f;                                     // input recorded at unity, no boost
    ...
    // BLEND LFO (page 2, POT2). LFO 1 drives Blend as a second destination, sharing its
    // rate and shape. BIPOLAR here, unlike the read-position destination: it sweeps either
    // side of wherever the Blend knob sits rather than anchoring at it. Squared depth for
    // fine control at the bottom. Note a Blend sweep crosses the dry position at centre,
    // so at real depth you hear dry come and go, which is inherent to the 3-way law.
    const float bdep = held_bdep_*held_bdep_;
    float ba = g_blend + lfo_a*bdep, bb = g_blend + lfo_b*bdep;
    ba = ba < 0.0f ? 0.0f : (ba > 1.0f ? 1.0f : ba);
    bb = bb < 0.0f ? 0.0f : (bb > 1.0f ? 1.0f : bb);
    float vga, dga, gga, vgb, dgb, ggb;
    LawBlendLive(ba, &vga, &dga, &gga);
    LawBlendLive(bb, &vgb, &dgb, &ggb);
    // Interpolated across the block rather than stepped, so a fast sweep cannot zipper.
    const float dvg = (vgb-vga)/(float)n, ddg = (dgb-dga)/(float)n, dgg = (ggb-gga)/(float)n;
    float voc_g = vga, dry_g = dga, grain_g = gga;
    ...
    lfo2_.SetSkew(0.35f + held_rate2_*0.15f);
    // RATE, 0.05 to 1.5 Hz exponential: 20 s per cycle at the slow end, 0.67 s at the
    // fast end. Previously 0.02 to 2 Hz, which spent the bottom third of the knob between
    // 50 s and 10 s where nothing audibly changes, and raced past the useful middle on the
    // way to a speed this pedal has no use for. Narrowing both ends keeps the mapping a
    // single exponential, so equal rotation still covers equal frequency ratio, but each
    // tenth of travel now spans 0.49 octaves instead of 0.66: the whole knob is 26% finer.
    const float lfo2_hz  = 0.05f*expf(held_rate2_*logf(1.5f/0.05f));
    ...
    const bool need_voc   = !empty && (vga > 1e-4f || vgb > 1e-4f);
    const bool need_grain = !empty && (gga > 1e-4f || ggb > 1e-4f);
    ...
    for (int i = 0; i < n; ++i) {
        ...
        voc_g += dvg; dry_g += ddg; grain_g += dgg;
        ...
        ing_[i] = d;                             // recorded copy, unity: no input boost
    }
    ...
    // shift or bandpass. Previously the tap sat after them, which meant an overdub
    // recorded already-shifted material and the next pass shifted it again: frequency
    // shifting is not idempotent, so layers drifted further out on every pass. Effects
    // are now monitoring and playback only, applied once, non-destructive.
    ...
    // both exponential, so the knob tracks pitch rather than linear frequency.
    {
        ...
        if (ar > 0.03f) {
            ...
            if (c.bpf_lfo) hz *= exp2f(lfo2_amt*lfo2_b*2.0f);
            if (hz < 40.0f)    hz = 40.0f;
            if (hz > 12000.0f) hz = 12000.0f;
            float mx = t < 0.15f ? t/0.15f : 1.0f;
            bpf_.Process(pre_, n, hz, mx);
        }
    }


    // Output is built last, from the fully effected mix.
    for (int i = 0; i < n; ++i) {
