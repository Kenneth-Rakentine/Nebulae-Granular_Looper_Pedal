...
void Engine::Init(float sr, float* a, float* b, int cap) {
    ...
    catch_[3].Init(1.0f,  0.0f,  0.0f);   // POT6: Blend   / LFO Depth    / PHASER MIX (0 = off)
    ...
    held_bpf_=0.5f; held_pmix_=0.0f; held_fshift_=0.5f; held_fsmix_=1.0f;
...
void Engine::Process(const float* in, float* out, int n, Controls& c) {
    ...
    if (held_pmix_ > 0.005f) {
        // Speed is not its own control: it tracks the SIZE LFO rate (POT2), so the loop
        // breathing and the notch climb move together instead of drifting against each
        // other. Feedback fixed low, since high resonance narrows each notch into a
        // whistle and fights the smooth handover the barber-pole illusion depends on.
        bph_.Process(pre_, n, lfo2_hz, 0.35f, held_pmix_);
    }
