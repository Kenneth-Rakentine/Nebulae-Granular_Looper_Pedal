// nebulae v1.33  (nebulae_engine.cpp)
...
void Engine::Init(float sr, float* a, float* b, int cap) {
    ...
    catch_[4].Init(0.5f,  0.0f,  0.0f);   // POT2: Speed / BLEND LFO DEPTH / SIZE LFO RATE
    ...
    held_rate_=0.0f;  held_depth_=0.0f; held_speed_=0.5f; held_bdep_=0.0f;
    held_pitch_=0.5f; held_outlvl_=0.0f;
    held_bdep_=0.5f;   // unity overdub level
...
void Engine::Process(const float* in, float* out, int n, Controls& c) {
    ...
    in_gain_ = 1.0f;                     // input recorded at unity; no dub level control
    ...
    // BLEND LFO (SHIFT+POT2). LFO 1 drives Blend as a second destination, sharing its rate
    // with read position but keeping its own depth. UNIPOLAR, matching the read-position
    // path: the offset only ever pushes upward from wherever the Blend knob sits, so the
    // knob stays the floor of the sweep and the motion is easy to follow. Squared depth
    // for fine control at the bottom, clamped so the sweep cannot run off the top.
    const float bl_uni = (lfo_prev_ + 1.0f)*0.5f;             // -1..1 -> 0..1
    float g_blend = held_blend_ + bl_uni*held_bdep_*held_bdep_;
    ...
    // both exponential, so the knob tracks pitch rather than linear frequency.
    {
        ...
        if (ar > 0.03f) {
            ...
            if (c.bpf_lfo) hz *= exp2f(lfo2_amt*lfo2_prev_*2.0f);
