...
void BuildWindows(float bank[W_BANK][WIN_LEN+1]) {
      ...
      Gen7(bank[W_RAMPDOWN], s, 5); }
    {
        ...
        // tail hangs on. Against the single exponential this reaches -6 dB at 11.9% of the
        // grain instead of 15.8% (snappier) while reaching -20 dB at 50.6% instead of
        // 37.7% (longer), with the mean level unchanged. The t^6 terminal taper only bites
        // near the very end, forcing the true zero that keeps grain boundaries silent
        // without disturbing the body of the curve.
        int m = WIN_LEN+1-a;
