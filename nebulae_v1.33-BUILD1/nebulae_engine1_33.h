// nebulae v1.33  (nebulae_engine.h)
...
// Deliberately the dull one. Lfo above is a phase-warped sine with character; this is the
// symmetric triangle a sampler gives you, bipolar, no skew and no output curve. When you
// want obvious motion rather than shape, this is the one to reach for. Computed per sample
// so its corners land exactly rather than up to a block late.
class TriLfo {
public:
    void  Init(float sr) { sr_ = sr; ph_ = 0.0; }
    inline float Step(float hz) {
        ...
        if (ph_ >= 1.0) ph_ -= 1.0;
        float p = (float)ph_;
        float v = (p < 0.5f) ? (p*2.0f) : (2.0f - p*2.0f);   // 0..1..0
        return v*2.0f - 1.0f;                                 // -1..+1
    }
private:
    float sr_ = 48000.f; double ph_ = 0.0;
};
...
class CatchPot {
public:
    void  Init(float initA, float initB, float initC = 0.5f);
    void  SetPage(int page);                 // 0 = primary, 1 = shift, 2 = shift + FS1 held
    ...
    // control until each was physically swept.
    void  ResetAlt();
private:
...
struct Controls {
    ...
    bool  bpf_lfo;   // page-3 latch: LFO 2 also modulates the bandpass
...
private:
    ...
    Recorder    rec_;
    Lfo         lfo_;
    TriLfo      lfo2_;     // lfo_ = read position, lfo2_ = loop size
    FreqShift   fsh_;
    BarberPhaser bph_;
    Bandpass     bpf_;
    PotSmoother sm_[6];
    ...
    float held_rate_, held_depth_, held_speed_, held_bdep_, held_pitch_, held_outlvl_;
    float held_rate2_, held_depth2_;               // page 3: size LFO rate / depth
