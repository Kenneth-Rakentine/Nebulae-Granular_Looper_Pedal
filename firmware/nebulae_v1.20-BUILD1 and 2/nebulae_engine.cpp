...
void Engine::Init(float sr, float* a, float* b, int cap) {
    ...
    //                 primary  shift   page3
    catch_[0].Init(0.0f,  0.0f,  0.2857f); // POT1: Start  / LFO Rate     / SIZE LFO SKEW
                                          //       knob 0.2857 -> skew 0.35, the shipped shape
    catch_[1].Init(1.0f,  0.0f,  0.0f);   // POT3: Size    / Grain Spray  / SIZE LFO DEPTH
    catch_[2].Init(0.55f, 0.5f,  0.5f);   // POT4: Density / Overlap      / BANDPASS (0.5 = off)
    ...
    catch_[5].Init(0.5f,  0.0f,  0.3f);   // POT5: Pitch   / Output Level / PHASER SPD
    ...
    held_rate2_=0.0f; held_depth2_=0.0f; held_skew2_=0.2857f; lfo2_prev_=0.0f;
    held_pspd_=0.3f; held_pmix_=0.0f; held_bpf_=0.5f;
...
void Engine::ResetAltParams() {
    ...
    held_rate2_ = 0.0f; held_depth2_ = 0.0f; held_skew2_ = 0.2857f;
    held_pspd_  = 0.3f;  held_pmix_ = 0.0f;  held_bpf_ = 0.5f;
}
...
void Engine::Process(const float* in, float* out, int n, Controls& c) {
    ...
    if (page == 0) { held_start_ = p1; held_speed_ = p2; held_size_ = p3;
                     held_dens_  = p4; held_pitch_ = p5; held_blend_ = p6; }
                          ...
                          held_skew2_ = p1; held_rate2_ = p2; held_depth2_ = p3;
                          held_bpf_   = p4; held_pspd_  = p5; held_pmix_  = p6; }
    ...
    // SIZE LFO SKEW (page 3: POT1). 0.15 to 0.85, defaulting to 0.35. Below centre the
    // rise is quick and the fall long, which with the 0.6 output curve gives the snap-out
    // then blossoming sink. Above centre mirrors it: slow swell, quick return. Centre is
    // an even sine.
    lfo2_.SetSkew(0.15f + held_skew2_*0.70f);
    ...
    if (held_pmix_ > 0.005f) {
        float spd = 0.02f*expf(held_pspd_*logf(2.0f/0.02f));   // 0.02 .. 2 Hz
        // Feedback fixed low. High resonance narrows each notch into a whistle and fights
        // the smooth handover that makes the barber-pole illusion work.
        bph_.Process(pre_, n, spd, 0.35f, held_pmix_);
    }

    // BANDPASS (page 3: POT4). Last in the chain, before output gain. Centre is off; the
