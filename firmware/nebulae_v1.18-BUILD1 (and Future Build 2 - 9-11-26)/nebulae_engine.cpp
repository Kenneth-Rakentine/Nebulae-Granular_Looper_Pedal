...
void Engine::Init(float sr, float* a, float* b, int cap) {
    ...
    catch_[2].Init(0.55f, 0.5f,  0.0f);   // POT4: Density / Overlap      / BARBER PHASER (0 = off)
    catch_[3].Init(1.0f,  0.0f,  1.0f);   // POT6: Blend   / LFO Depth    / SHIFT MIX (1.0 = full wet)
    ...
    held_rate2_=0.0f; held_depth2_=0.0f; held_fshift_=0.5f; held_fsmix_=1.0f; held_skew2_=0.2857f; held_phase_=0.0f; lfo2_prev_=0.0f;
...
void Engine::ResetAltParams() {
    ...
    held_rate2_ = 0.0f; held_depth2_ = 0.0f; held_fshift_ = 0.5f; held_fsmix_ = 1.0f;
    held_skew2_ = 0.2857f; held_phase_ = 0.0f;
}
...
void Engine::Process(const float* in, float* out, int n, Controls& c) {
    ...
    if (page == 0) { held_start_ = p1; held_speed_ = p2; held_size_ = p3;
                     held_dens_  = p4; held_pitch_ = p5; held_blend_ = p6; }
                          ...
                          held_phase_ = p4; held_fshift_ = p5; held_fsmix_ = p6; }
    ...
    // FREQ SHIFT (page 3: POT5). OFF at centre with a dead zone, then EXPONENTIAL
    // 0.5 Hz to 100 Hz each side. The earlier 2 Hz to 2 kHz range put everything past
    // about 3:30 into heavily inharmonic territory that reads as a resonant sweep rather
    // than a timbral shift, so two thirds of the knob were unusable. 100 Hz keeps a little
    // headroom past the useful ceiling and spreads the rest of the travel across the
    // 1 to 30 Hz zone where sidebands colour the tone without destroying pitch.
    // Applied to the pre-gain mix so monitoring and recording get the same thing.
    {
        ...
        if (ar > 0.03f) {
            ...
            hz = 0.5f*expf(t*logf(200.0f));                // 0.5 Hz .. 100 Hz
            if (r < 0.0f) hz = -hz;
        }
        fsh_.Process(pre_, n, hz, held_fsmix_);

        // BARBER-POLE PHASER (page 3: POT4). One knob for the whole effect: CCW is off,
        // and turning up raises sweep rate, feedback and mix together along a curve that
        // keeps them musically matched. Slow and shallow at the bottom, fast and resonant
        // at the top. Split across three knobs if it earns the panel space.
        if (held_phase_ > 0.01f) {
            float t  = held_phase_;
            float rt = 0.03f*expf(t*logf(60.0f));    // 0.03 .. 1.8 Hz
            float fb = 0.45f + t*0.45f;              // 0.45 .. 0.90
            float mx = t < 0.25f ? t*4.0f : 1.0f;    // fades in over the first quarter
            bph_.Process(pre_, n, rt, fb, mx);
        }
