                          ...
                          AudioHandle::OutputBuffer out, size_t size)
{
    ...
    // ---- FS1: bypass toggle ----
    if (fs[0].RisingEdge()) bypass = !bypass;
    ...
    ctrl.shift   = sw[0].Pressed();   // SW1 SHIFT  : POT3 -> Window, POT4 -> Overlap
    ctrl.src_mix = sw[1].Pressed();   // SW2 SRC    : MIX (overdub) / IN (clean input)
    ctrl.expo    = sw[2].Pressed();   // SW3 EXPO   : D9 expodec / stock RampDown
    ctrl.freeze  = sw[3].Pressed();   // SW4 FREEZE : held / run  (kspeed = 0)
    ...
    else led[1].Write(recording || (ctrl.shift && engine.CatchDir(0) != 0));
}
