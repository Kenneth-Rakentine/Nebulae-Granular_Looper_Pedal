// nebulae v2.0-BUILD2-PRE  (nebulae_engine.cpp)
// nebulae v1.32  (nebulae_engine.cpp)
...
float Lfo::Tick(float hz, int n) {
    ...
    float v = sinf(2.0f*(float)M_PI*w);
    if (curve_ != 1.0f) v = (v < 0.0f ? -1.0f : 1.0f)*powf(fabsf(v), curve_);
    last_ = v;
    return last_;
}

// ============================ CatchPot ============================
void CatchPot::Init(float a, float b, float c, float d, float e) {
    val_[0]=a; val_[1]=b; val_[2]=c; val_[3]=d; val_[4]=e;
    init_[0]=a; init_[1]=b; init_[2]=c; init_[3]=d; init_[4]=e;
    ...
    entry_=0.0f; moved_=0.0f; ramp_=1.0f; gaveup_=false;
}
void CatchPot::SetPage(int p) {
    ...
    entry_ = last_; moved_ = 0.0f; gaveup_ = false;
    ramp_  = caught_[p] ? 1.0f : 0.0f;
...
void CatchPot::ResetAlt() {
    for (int p = 1; p < NP; ++p) {
        val_[p]  = init_[p];
        caught_[p] = false;          // must be swept through again to pick up
        park_[p] = -1.0f;
        left_caught_[p] = false;
    }
}
float CatchPot::Update(float raw) {
    const int p = page_;
    // GIVE-UP THRESHOLD: 15% of knob travel. Beyond that the knob has clearly been grabbed
    // on purpose, so waiting for a crossing that may be 70% away serves no one.
    const float GIVEUP = 0.15f;
    const float RAMP   = 0.02f;          // ~25 ms at 94 blocks/s: a fast fade, not a click
    if (!caught_[p] && p > 0) {
        moved_ += fabsf(raw - (have_last_ ? last_ : raw));
        if (!gaveup_ && moved_ > GIVEUP) { gaveup_ = true; ramp_ = 0.0f; }
    }
    if (!caught_[p]) {
        ...
        if (caught_[p]) ramp_ = 1.0f;       // ordinary crossing: no ramp needed
        dir_ = (caught_[p] || gaveup_) ? 0 : (d > 0.0f ? -1 : +1);
    } else dir_ = 0;
    if (caught_[p]) { val_[p] = raw; ramp_ = 1.0f; }
    else if (gaveup_) {
        // Took over without a crossing. Ramp the stored value to the knob so the handover
        // is a fast fade rather than a step, then behave exactly like a caught pot.
        ramp_ += RAMP;
        if (ramp_ >= 1.0f) { ramp_ = 1.0f; caught_[p] = true; }
        val_[p] = val_[p] + (raw - val_[p])*ramp_;
    }
...
void Engine::Init(float sr, float* a, float* b, int cap) {
    ...
    // LFO 2 is the plain sampler LFO: skewed triangle, its skew on its own knob rather
    // than riding the rate as it did on build 1. Bipolar, so loop length swings both above
    // and below wherever Size is set.
    lfo2_.Init(sr); lfo2_.SetSkew(0.5f);
    fsh_.Init(sr);
    bph_.Init(sr);
    bpf_.Init(sr);
    dc_.Init(sr, 12.0f);
    ...
    //                 primary  shift   page3
    // Init order: primary, shift, tertiary, LFO 1 page, LFO 2 page.
    // Every LFO depth inits to 0.5, which is CENTRE of a bipolar attenuverter and means
    // zero depth. That is what keeps depth knobs off the extremes the catch trap punishes.
    catch_[0].Init(0.0f,  0.5f,  0.5f,  0.5f, 0.5f);  // POT1 Start  /TAPE DECAY/BANDPASS/dep read pos
    catch_[1].Init(1.0f,  0.0f,  0.5f,  0.5f, 0.5f);  // POT3 Size   /Spray     /WINDOW  /dep loop len
    catch_[2].Init(0.55f, 0.5f,  1.0f,  0.5f, 0.5f);  // POT4 Density/Overlap   /SHIFT MIX/dep bandpass
    catch_[3].Init(1.0f,  0.0f,  0.0f,  0.5f, 0.5f);  // POT6 Blend  /WIN DECAY /PITCH RND/dep blend
    catch_[4].Init(0.5f,  0.5f,  0.0f,  0.0f, 0.0f);  // POT2 Speed  /Dub lvl   /BPF Q   /LFO RATE
                                          //       (dub 0.5 = unity on the 0.25x..4x curve)
    catch_[5].Init(0.5f,  0.0f,  0.5f,  0.65f,0.5f);  // POT5 Pitch  /Output    /FREQ SHFT/LFO SKEW
    ...
    held_rate2_=0.0f; held_depth2_=0.0f; held_skew2_=0.5f;
    held_bpf_=0.5f; held_prand_=0.0f; held_fshift_=0.5f; held_fsmix_=1.0f;
...
void Engine::ResetAltParams() {
    for (int i = 0; i < 6; ++i) catch_[i].ResetAlt();
    // The held_ values mirror the catch outputs and are only refreshed on the page that
    // owns them, so a reset has to clear them directly or the old values persist until
    // you next visit that page.
    held_rate_ = 0.0f; held_inlvl_ = 0.5f; held_spray_ = 0.0f;
    held_ovl_  = 0.5f; held_outlvl_ = 0.0f; held_depth_ = 0.0f;
    held_rate2_ = 0.0f; held_depth2_ = 0.0f;
    held_bpf_   = 0.5f; held_prand_ = 0.0f; held_fshift_ = 0.5f; held_fsmix_ = 1.0f;
}
...
void Engine::Process(const float* in, float* out, int n, Controls& c) {
    ...
    else if (page == 1) { held_tape_  = p1; held_inlvl_ = p2; held_spray_ = p3;
                          held_ovl_   = p4; held_outlvl_= p5; held_windec_= p6; }
    else if (page == 2) { /* tertiary: FS1 held. No panel labels. */
                          held_bpf_   = p1; held_bpq_   = p2; held_win_    = p3;
                          held_fsmix_ = p4; held_fshift_= p5; held_prand_  = p6; }
    else if (page == 3) { /* LFO 1 page: button red */
                          d_rd1_ = (p1-0.5f)*2.0f; held_rate_  = p2; d_sz1_ = (p3-0.5f)*2.0f;
                          d_bpf1_= (p4-0.5f)*2.0f; held_skew_  = p5; d_bl1_ = (p6-0.5f)*2.0f; }
    else                { /* LFO 2 page: button green */
                          d_rd2_ = (p1-0.5f)*2.0f; held_rate2_ = p2; d_sz2_ = (p3-0.5f)*2.0f;
                          d_bpf2_= (p4-0.5f)*2.0f; held_skew2_ = p5; d_bl2_ = (p6-0.5f)*2.0f; }
    ...
    // BLEND, with both LFOs able to reach it. Bipolar attenuverters, signed square so the
    // bottom of each knob is fine-grained. A blend sweep crosses the dry position at
    // centre, which is inherent to the stock three-way law, so depth here is worth keeping
    // modest unless dry coming and going is the effect you want.
    const float bl_mod = d_bl1_*fabsf(d_bl1_)*lfo_prev_ + d_bl2_*fabsf(d_bl2_)*lfo2_prev_;
    float g_blend = held_blend_ + bl_mod;
    g_blend = g_blend < 0.0f ? 0.0f : (g_blend > 1.0f ? 1.0f : g_blend);
    ...
    // WINDOW (page 3: POT3). The stock continuous sweep across the whole seven-window
    // bank, restored. SW3 no longer picks the window: it chooses what slot 2 resolves to,
    // expodec or the stock RampDown, exactly as WinMix was always written to do.
    // Rebuilding the whole bank costs a few thousand exp/pow calls, so only do it when
    // the knob has actually moved. Expodec is the only window the decay knob touches.
    if (fabsf(held_windec_ - built_decay_) > 1e-4f) {
        BuildWindows(bank_, held_windec_);
        built_decay_ = held_windec_;
    }
    ...
    lfo_.SetSkew(0.35f + held_skew_*0.60f);      // LFO 1 skew now on its own knob
    const float lfo_amt = d_rd1_*fabsf(d_rd1_);  // signed square: fine at the bottom, bipolar
    ...
    // SIZE LFO shape tracks its own rate. At the slow end the asymmetry is audible as
    // motion, so skew sits at 0.35 (quick rise, long sink). As the rate climbs that
    // shape stops reading as shape and just colours the wobble, so it eases toward 0.50,
    // a plain sine. One knob sets both.
    // LFO 2 RATE. Free mode is 0.1 to 15 Hz exponential, which reaches a fast trill
    // without pretending to be audio rate: the control-rate Nyquist is 46.9 Hz, and the
    // bandpass destination updates per block regardless, so beyond about 15 Hz a saw stops
    // being a saw. In DIVISION mode the rate is instead a whole-number ratio of the loop,
    // so every cycle completes exactly at the loop boundary.
    lfo2_.SetSkew(held_skew2_);
    float lfo2_hz;
    if (c.lfo2_div) {
        // Seven divisions across the knob. Locking the period to the loop is what lets the
        // phase reset at the wrap without a discontinuity: an unsynced LFO reset mid-cycle
        // steps its output, which clicks on loop length or filter frequency.
        static const float DIV[7] = { 0.25f, 0.5f, 1.0f, 2.0f, 4.0f, 8.0f, 16.0f };
        int d = (int)(held_rate2_*6.999f);
        lfo2_hz = DIV[d] / (lsize > 1e-4f ? lsize : 1e-4f);
        if (lfo2_hz > 40.0f) lfo2_hz = 40.0f;     // stay under the control-rate Nyquist
    } else {
        lfo2_hz = 0.1f*expf(held_rate2_*logf(15.0f/0.1f));
    }
    const float lfo2_amt = d_sz2_*fabsf(d_sz2_);   // LFO 2 to loop length, bipolar
    float lfo2_last = 0.0f;

    for (int i = 0; i < n; ++i) {
        float l2  = lfo2_.Step(lfo2_hz);          // per sample: corners land exactly
        lfo2_last = l2; lfo2_prev_ = l2;
        float lsz = lsize*exp2f(lfo2_amt*l2 + d_sz1_*fabsf(d_sz1_)*lfo_b);
        double finc = ((1.0/lsz)*speed_)/sr_;
        double prev_ph = phase_;
        phase_ += finc;
        while (phase_ >= 1.0) phase_ -= 1.0;
        while (phase_ <  0.0) phase_ += 1.0;
        // DIVISION mode retriggers LFO 2 when the playhead wraps the loop, so the
        // modulation lands in the same place every pass and becomes part of the loop's
        // rhythm rather than drifting against it.
        if (c.lfo2_div && phase_ < prev_ph) lfo2_.SetPhase(0.0);
    ...
    // BARBER-POLE PHASER (page 3: POT4 speed, POT5 feedback, POT6 mix).
    // Four parallel six-stage allpass chains sweep the same exponential range with their
    // LFO phases staggered, each amplitude-windowed so it is silent at both ends of its
    // travel. Around 24 notches slide upward at once and never sweep back: as the top
    // group fades out a fresh group is already climbing from the bottom. The four
    // staggered raised cosines sum to exactly 2.0, so the handover has no amplitude
    // ripple at all. Applied to the pre-gain mix, so overdubs record it.
    ...
    // RECORD TAP. Taken BEFORE the page-3 effects, so the buffer never contains freq
    // shift or bandpass. With the tap after them, an overdub recorded already-shifted
    // material and the next pass shifted it again: frequency shifting is not idempotent,
    // so layers drifted further out on every pass. Effects are now monitoring and
    // playback only, applied once, non-destructive.
    ...
    // FREQ SHIFT (page 3: POT5 amount, POT4 mix). Hilbert SSB, one sideband only, which
    // is what makes it inharmonic rather than ring mod. Dead zone at centre, then
    // exponential 0.5 Hz to 100 Hz each side: most of the travel sits in the 1 to 30 Hz
    // zone where sidebands colour the tone without destroying pitch.
    {
        float r  = (held_fshift_ - 0.5f)*2.0f;
        float ar = fabsf(r);
        float hz = 0.0f;
        if (ar > 0.03f) {
            float t = (ar - 0.03f)/0.97f;
            hz = 0.5f*expf(t*logf(200.0f));
            if (r < 0.0f) hz = -hz;
        }
        fsh_.Process(pre_, n, hz, held_fsmix_);
    }

    bpf_mod_ = d_bpf1_*lfo_b + d_bpf2_*lfo2_last;   // octaves, bipolar attenuverters

    // BANDPASS (page 3: POT1). Last in the chain, before output gain. Centre is off; the
    // wet amount fades in over the first 15% of travel either side so there is no jump at
    // the dead zone. CCW sweeps down from 900 Hz to 120 Hz, CW up from 900 Hz to 6 kHz,
    // both exponential, so the knob tracks pitch rather than linear frequency.
    {
        float r  = (held_bpf_ - 0.5f)*2.0f;                    // -1 .. +1
        float ar = fabsf(r);
        if (ar > 0.03f) {
            float t  = (ar - 0.03f)/0.97f;                     // 0 .. 1 outside dead zone
            float hz = (r < 0.0f) ? 900.0f*expf(-t*logf(900.0f/120.0f))
                                  : 900.0f*expf( t*logf(6000.0f/900.0f));
            // LFO 2 is global: when latched on (page 3 + SW2 flip) it modulates the
            // bandpass as well as loop length, sharing one rate, depth and shape.
            // Bipolar and exponential, +/- 2 octaves at full depth, so the sweep
            // tracks pitch and stays centred on whatever POT1 is set to. Depth 0
            // leaves the filter still, so the latch costs nothing until you use it.
            hz *= exp2f(bpf_mod_);          // set by the LFO depth routing below
            if (hz < 40.0f)    hz = 40.0f;
            if (hz > 12000.0f) hz = 12000.0f;
            float mx = t < 0.15f ? t/0.15f : 1.0f;
            bpf_.Process(pre_, n, hz, mx, 1.0f + held_bpq_*3.0f);
        }
    }

    // Re-apply output gain and limit for the monitored copy.
    for (int i = 0; i < n; ++i) {
