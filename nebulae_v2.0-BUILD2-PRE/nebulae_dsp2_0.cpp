// nebulae v2.0-BUILD2-PRE  (nebulae_dsp.cpp)
// nebulae v1.32  (nebulae_dsp.cpp)
...
void BuildWindows(float bank[W_BANK][WIN_LEN+1], float decay) {
      ...
      Gen7(bank[W_RAMPDOWN], s, 5); }
    {
        ...
        // DECAY, morphed by the WINDOW DECAY knob. At knob 0 the fast component IS the
        // whole curve with its time constant set to 1/ln(1000), which is exactly 0.001^t:
        // bit-identical to the original single exponential, not an approximation of it.
        // Turning up introduces a slow second component, so the transient sharpens while
        // the tail stretches. That pairing is inherent: a single exponential cannot get a
        // longer tail without softening, so lengthening needs a second stage and that
        // stage is what steepens the front.
        //
        // The terminal taper forces the curve to land on exactly 0.001 whatever the knob
        // does. It is written as (target/actual)^(t^6) so it is exactly 1.0 at knob 0,
        // where actual already equals target, and only bites near the very end elsewhere.
        int m = WIN_LEN+1-a;
        const double LN1000 = log(1000.0);
        const double A  = 1.0 - 0.35*(double)decay;
        const double T1 = 1.0/LN1000 - (1.0/LN1000 - 0.05)*(double)decay;
        const double T2 = 0.40;
        const double endv = A*exp(-1.0/T1) + (1.0-A)*exp(-1.0/T2);
        for (int i = 0; i < m; ++i) {
            double t = (double)i/(m-1);
            double v = A*exp(-t/T1) + (1.0-A)*exp(-t/T2);
            v *= pow(0.001/endv, pow(t, 6.0));
            ft[a+i] = (float)v;
        }
    ...
    for (int s = 0; s < n; ++s) {
        ...
        if (grainphase_ >= 1.0) {
            ...
            if (dur >= 1.0) {
                ...
                // GRAIN PITCH RANDOMISATION, assigned once at launch and held for the
                // grain's life. Stock is rand(amt*1200), bipolar cents, so +/- an octave
                // at full: far too wide to stay musical. Squared curve to +/-120 cents
                // instead, which keeps most of the travel in genuine detune territory
                // (about +/-8 cents at quarter travel, +/-30 at half) and tops out just
                // past a semitone. Per grain, not per sample: per sample would be FM.
                double dl = (wavfreq/sr_)*tablen;
                if (pitchrand > 1e-4f) {
                    float cents = Rnd()*pitchrand*pitchrand*120.0f;
                    dl *= pow(2.0, cents/1200.0);
                }
                double envinc = 1.0/dur;
                g_[slot] = { ph*tablen, dl, pc*envinc, envinc,
...
// ============================ FREQ SHIFT ============================
void FreqShift::Init(float sr) {
    sr_ = sr; ph_ = 0.f; z1_ = 0.f; wet_ = 0.f; hz_ = 0.f;
    const float ci[4] = {0.47940086558884f,0.87621849353931f,0.97659758950819f,0.99749925593555f};
    const float cq[4] = {0.16175849836770f,0.73302893234149f,0.94534970032911f,0.99059915668453f};
    for (int k=0;k<4;++k){ i_[k]=AP2(); q_[k]=AP2(); i_[k].c=ci[k]; q_[k].c=cq[k]; }
}
void FreqShift::Process(float* buf, int n, float hz, float mix) {
    // mix blends the shifted signal against dry. Hearing both together is the classic
    // Bode usage: the two beat against each other, which reads as movement rather than
    // the flat detune of a fully wet shift. At mix 1.0 this is bit-identical to before.
    float wt = (hz == 0.0f) ? 0.0f : mix;
    const float dh=(hz-hz_)/(float)n, dw=(wt-wet_)/(float)n;
    float h=hz_, w=wet_;
    for (int s=0;s<n;++s){
        h+=dh; w+=dw;
        float x=buf[s], ai=x, aq=x;
        for(int k=0;k<4;++k) ai=i_[k].P(ai);
        for(int k=0;k<4;++k) aq=q_[k].P(aq);
        float aI=z1_; z1_=ai;                        // 1-sample delay for quadrature accuracy
        ph_ += h/sr_; ph_ -= floorf(ph_);
        float c=cosf(2.0f*(float)M_PI*ph_), sn=sinf(2.0f*(float)M_PI*ph_);
        // PLUS: with this chain assignment, minus yields the LOWER sideband. Verified
        // spectrally: +100 Hz in -> 400 Hz out from a 300 Hz tone, 46 dB image rejection.
        buf[s] = x*(1.0f-w) + (aI*c + aq*sn)*w;
    }
    hz_=hz; wet_=wt;
}

// ============================ BARBER-POLE PHASER ============================
void BarberPhaser::Init(float sr) {
    sr_ = sr; ph_ = 0.0f; mix_ = 0.0f;
    for (int c = 0; c < NCH; ++c) { fb_[c] = 0.0f;
        for (int k = 0; k < NAP; ++k) ap_[c][k] = AP1(); }
}
void BarberPhaser::Process(float* buf, int n, float rate_hz, float feedback, float mix) {
    const float FMIN = 120.0f, FMAX = 6000.0f;
    const float lr   = logf(FMAX/FMIN);
    const float dmix = (mix - mix_)/(float)n;
    float m = mix_;
    // Coefficients move slowly compared with audio, so they are recomputed once per block
    // rather than per sample. At 512 samples that is ~94 Hz, far above any sweep rate.
    float a[NCH][NAP], w[NCH];
    float wsum = 0.0f;
    for (int c = 0; c < NCH; ++c) {
        float p = ph_ + (float)c/(float)NCH;
        p -= floorf(p);
        float f0 = FMIN*expf(p*lr);
        float rc = 0.5f*(1.0f - cosf(2.0f*(float)M_PI*p));
        w[c] = rc*rc;                          // squared: sharper, sum still exactly constant
        wsum += w[c];
        for (int k = 0; k < NAP; ++k) {
            // stages spread over a half-octave so the notches are not all coincident
            float f = f0*(1.0f + 0.12f*(float)k);
            if (f > sr_*0.45f) f = sr_*0.45f;
            float t = tanf((float)M_PI*f/sr_);
            a[c][k] = (t - 1.0f)/(t + 1.0f);
        }
    }
    if (wsum < 1e-6f) wsum = 1e-6f;
    const float fbk = feedback > 0.95f ? 0.95f : (feedback < 0.0f ? 0.0f : feedback);
    for (int i = 0; i < n; ++i) {
        m += dmix;
        float x = buf[i], wet = 0.0f;
        for (int c = 0; c < NCH; ++c) {
            float v = x + fb_[c]*fbk;
            for (int k = 0; k < NAP; ++k) v = ap_[c][k].P(v);
            fb_[c] = v;
            wet += v*w[c];
        }
        wet /= wsum;
        buf[i] = x*(1.0f - m) + 0.5f*(x + wet)*m;        // dry + allpass sum = notches
    }
    ph_ += rate_hz*(float)n/sr_;  ph_ -= floorf(ph_);
    mix_ = mix;
}

// ============================ BANDPASS ============================
void Bandpass::Process(float* buf, int n, float hz, float mix, float Q) {
    // Q from the panel, 1.0 fully CCW (the shipped value) to 4.0 fully CW. This topology
    // never self-oscillates: damping is 1/Q and only reaches zero at infinite Q, so it
    // rings progressively longer instead. Ring time in CYCLES is constant, so the low end
    // of the sweep is the worst case: the same Q rings 50x longer at 120 Hz than at 6 kHz.
    // Q=4 is 73 ms at 120 Hz, audibly resonant but not pitched; Q=6 is 110 ms, a drone.
    if (Q < 0.5f) Q = 0.5f; else if (Q > 4.0f) Q = 4.0f;
    const float df = (hz - f_)/(float)n, dm = (mix - mix_)/(float)n;
    float f = f_, m = mix_;
    for (int i = 0; i < n; ++i) {
        f += df; m += dm;
        float g  = tanf((float)M_PI*f/sr_);
        float k  = 1.0f/Q;
        ...
        ic1_ = 2.0f*v1 - ic1_;
        ic2_ = 2.0f*v2 - ic2_;
        // A bandpass throws away everything outside its passband, so the perceived level
        // drops even though the peak itself is unity. Makeup scales with wet amount only,
        // reaching about +5 dB fully engaged, so the filtered signal stays present rather
        // than receding. Deliberately modest: enough to hear, not enough to need its own
        // limiter.
        const float makeup = 1.0f + m*0.8f;
        buf[i] = x*(1.0f - m) + v1*m*makeup;   // v1 is the bandpass tap
    }
    f_ = hz; mix_ = mix;
}
