...
void Engine::Init(float sr, float* a, float* b, int cap) {
    ...
    catch_[1].Init(1.0f,  0.0f);    // POT3: Size    / Grain Spray
    catch_[2].Init(0.55f, 0.5f);    // POT4: Density / Overlap
    catch_[3].Init(1.0f,  0.0f);    // POT6: Blend   / LFO Depth
    catch_[4].Init(0.5f,  0.5f);    // POT2: Speed / Overdub Level -- 0.5 = UNITY on the
                                    // symmetric 0.25x..4x curve. Was 0.0, which meant
                                    // unity on the old input-level curve but -12 dB here.
...
void Engine::Process(const float* in, float* out, int n, Controls& c) {
    ...
    else           { held_rate_  = p1; held_inlvl_ = p2; held_spray_ = p3;
